...
def findseq(needle, haystack, selName=None, het=0, firstOnly=0):
    ...
    # remove redundant residues, Longxing 2019-01-26
    curr_res = None
    for index, v in enumerate(aaDict['aaList']):
        if v == curr_res:
            aaDict['aaList'].pop(index)
        curr_res = v
    # 2019-01-26, Longxing, some pdbs contain non number residue index
    # just add a filter before converting that to a number.
    #IDs = [int(x[0]) for x in aaDict['aaList']]
    IDs = [int(filter( lambda ch: ch in '0123456789', x[0]) ) for x in aaDict['aaList']]
